...
TAREA: Busca "${brand} ${sku}" en Google y devuelve SOLO este JSON (sin markdown ni texto extra):
{
  ...
  app.post("/api/q/2", geminiLimiter, requireLicense as any, requireFeature("academia") as any, lenguetaLimiter as any, checkDailyLimit as any, async (req: AuthRequest, res) => {
  ...
  app.get("/api/q/result/:jobId", requireLicense as any, requireFeature("academia") as any, async (req: AuthRequest, res) => {
...
Genera la imagen editada.`;
   ...
   * Confirma que la IA está disponible para la extensión. NO devuelve la key.
